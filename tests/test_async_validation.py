...
async def test_event_loop_heartbeat_under_load():
    """
    検証: データベースの重いI/O処理中もイベントループがブロックされていないことを証明する。
    
    仕組み:
    1. 非常に重い書き込み処理（1000件のエンティティ）を非同期で開始する。
    2. 同時に、短い間隔で『心拍（ハートビート）』を刻むタスクを走らせる。
    3. もしデータベース処理がイベントループをブロック（同期実行）している場合、ハートビートの間隔に大きな遅延が生じる。
    ...
    async def heavy_io_task():
        entities = [{"name": f"Stress_{i}", "description": "X" * 1000} for i in range(500)]
    ...
    avg_latency = sum(heartbeat_latencies) / len(heartbeat_latencies) if heartbeat_latencies else 0
...
async def test_concurrent_agent_limit_verification():
    ...
    async def agent_action(agent_id):
        return await save_memory_core(
            entities=[{"name": f"Concurrent_{agent_id}", "description": "Testing concurrency"}],
    ...
    async with await async_get_connection() as conn:
        cursor = await conn.execute("SELECT COUNT(*) FROM entities WHERE name LIKE 'Concurrent_%'")
