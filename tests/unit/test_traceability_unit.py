...
async def test_check_conflict_detected_unit(mock_gemini_globally):
    """
    Unit test: Verify conflict detection yields 'True' when
    Gemini detects conflict.
    """
    ...
    async with await async_get_connection() as conn:
        cursor = await conn.execute(
            "SELECT reason FROM conflicts WHERE entity_name = ?", ("Apple",)
        )
...
async def test_audit_log_metadata_persistence_unit(mock_gemini_globally):
    """
    Unit test: Verify that saving an observation creates a detailed
    audit log with meta_data.
    """
    # 1. Setup entities
    async with await async_get_connection() as conn:
    ...
    async with await async_get_connection() as conn:
        await save_observations(
            [{"entity_name": "ProjectX", "content": "Started today"}],
            "agent_007",
            conn,
        )
        await conn.commit()
    ...
    async with await async_get_connection() as conn:
        cursor = await conn.execute(
            "SELECT meta_data FROM audit_logs WHERE content_id = ? "
        ...
        assert meta["conflict_info"] is None
