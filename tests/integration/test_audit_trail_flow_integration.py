...
@pytest.mark.asyncio
async def test_audit_trail_full_flow_integration(mock_llm):
    """
    Integration test: Verify the full flow from save_memory to audit log persistence.
    ...
    async with await async_get_connection() as conn:
        cursor = await conn.execute(
            "SELECT action, meta_data FROM audit_logs WHERE content_id = ? AND table_name = 'observations' ORDER BY timestamp DESC",
            ("Kernel-v2",)
        )
...
async def test_save_memory_transactional_integrity_integration(mock_llm):
    ...
    """
    # Trigger an error during observation saving
    # (By passing malformed observations)
