...
import json
import asyncio
from shared_memory import thought_logic, logic, graph
...
async def test_reasoning_synergy_and_distillation(mock_llm):
    ...
    distill_result = {
        "entities": [{"name": "CoreEngine", "entity_type": "component", "description": "The main orchestrator."}],
        "relations": [],
    ...
    assert any(e["name"] == "CoreEngine" for e in search_a["graph"]["entities"]), "CoreEngine should be distilled from Session A"
    assert any("WAL mode" in o["content"] for o in search_a["graph"]["observations"]), "Observation should be distilled"
    ...
    assert any("CoreEngine" in str(k) or "WAL mode" in str(k) for k in rel_k), "Session B should salvage knowledge from Session A"

    # --- STEP 3: Final Distillation on Closure ---
    # Mock the final distillation report
    final_report = {
        "entities": [{"name": "SharedMemoryServer", "entity_type": "system", "description": "A robust MCP server."}],
        "relations": [{"source": "CoreEngine", "target": "SharedMemoryServer", "relation_type": "part_of", "justification": "Primary component"}],
        "observations": []
    }
    ...
    assert any(e["name"] == "SharedMemoryServer" for e in search_final["graph"]["entities"]), "Final distillation should save the system entity"
