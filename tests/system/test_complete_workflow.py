import pytest
import json
from shared_memory.logic import save_memory_core, read_memory_core, synthesize_entity, get_value_report_core
...
async def test_complete_user_workflow(mock_llm):
    ...
    mock_llm.models.set_response("generate_content", "Synthesized summary: This entity is a test node with multiple observations.")

    # 1. Build context
    await save_memory_core(
        entities=[{"name": "ProjectX", "description": "A top-secret project"}],
        agent_id=agent
    )
...
async def test_error_resilience_malformed_input(mock_llm):
    ...
    result = await save_memory_core(
        entities=[{"name": huge_string, "description": "Massive name"}],
        agent_id="test_agent"
    )
    assert "Saved" in result
    
    # Missing required keys in list
    result2 = await save_memory_core(
        entities=[{"not_a_name": "oops"}],
        agent_id="test_agent"
    )
