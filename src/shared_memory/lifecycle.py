...
async def manage_knowledge_activation_logic(ids: list[str], status: str):
    ...
    async with await async_get_connection() as conn:
        ...
        try:
            for table in tables:
                id_col = "name" if table == "entities" else "filename" if table == "bank_files" else "id"
                
                if table in ["relations", "observations"] and all(isinstance(i, str) for i in ids):
                    # For relations/observations, if IDs are entity names, deactivate all related items
                    if table == "observations":
                        ...
                        placeholders = ",".join(["?"] * len(ids))
                        cursor = await conn.execute(
                            f"UPDATE {table} SET status = ? WHERE subject IN ({placeholders}) OR object IN ({placeholders})",
                            [status] + ids + ids
                        )
                        changes += cursor.rowcount
                else:
...
async def list_inactive_knowledge_logic():
    ...
    """
    async with await async_get_connection() as conn:
        results = {"entities": [], "relations": [], "observations": [], "bank_files": []}
        
        # Entities
        cursor = await conn.execute("SELECT name, entity_type, status FROM entities WHERE status != 'active'")
        results["entities"] = [dict(row) for row in await cursor.fetchall()]
        
        # Relations
        cursor = await conn.execute("SELECT subject, predicate, object, status FROM relations WHERE status != 'active'")
        results["relations"] = [dict(row) for row in await cursor.fetchall()]
        
        # Observations
        cursor = await conn.execute("SELECT id, entity_name, content, status FROM observations WHERE status != 'active'")
        results["observations"] = [dict(row) for row in await cursor.fetchall()]
        
        # Bank Files
        cursor = await conn.execute("SELECT filename, status FROM bank_files WHERE status != 'active'")
...
async def run_knowledge_gc_logic(age_days: int = 180, dry_run: bool = False):
    ...
    async with await async_get_connection() as conn:
        ...
        cursor = await conn.execute(
            ...
            "julianday('now') - julianday(last_accessed) > ? AND importance_score < 0.1",
            (age_days,)
        )
        ...
        if dry_run:
            return f"Dry Run: Found {len(stale_ids)} items as candidates for deactivation: {stale_ids[:5]}..."
