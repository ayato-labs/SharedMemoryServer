...
| **データ漏洩リスク** | △ クラウド送信あり | ✅ 100% ローカル完結 |
 ...
 :::details なぜ LLM 自体を内蔵（エッジ LLM）せず、外部プロバイダーとの連携を選んだのか？
 開発過程で、軽量 LLM（Phi-3 等）をハブに直接埋め込む実験も行いました。しかし、以下の理由から「外部の Ollama や Gemini を検知・利用する」という **Plan A 戦略** に舵を切りました。
 
 1. **JSON Mode の壁**: 知識の蒸留には厳格な JSON 出力が必須ですが、これを 3B〜8B クラスの超小型モデルで、かつ推論エンジンへの複雑な依存なしに安定させるのは困難でした。実用的な JSON 推論には 26B 前後の規模が必要であり、それは「軽量なバックグラウンドサービス」という Ripen の設計思想と衝突します。
 2. **FastEmbed の効率性**: ベクトル検索（RAG）の心臓部には、100MB 級の超軽量モデル **FastEmbed** を採用しました。LLM で無理やりベクトルを計算するより圧倒的に高速で省メモリです。
 3. **システムの堅牢性**: ハブ自体は「知識の運び屋」として徹底的に安定させ、重い推論処理は外部の最適化されたランタイム（Ollama 等）に任せることで、IDE の動作を妨げない低レイテンシを維持しています。
 :::

ローカルの `FastEmbed` と `Ollama` を組み合わせることで、「AI が考える前に、記憶がそこにある」という次元のレスポンス速度を実現しています。
 
 :::message
 **ステータス監視**: ダッシュボード（`/dashboard`）から、AI Brain（LLM）がオンラインかどうかをリアルタイムで確認できます。万が一 Ollama が止まっていても、ハブはクラッシュせず、正常な検索機能を維持したまま警告を表示します。
 :::
