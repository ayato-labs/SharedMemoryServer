...
- **Professional License**: 商用利用およびチーム利用向けの標準プラン。
  ...
  - **現在は「無料」で提供中**: AGPL-3.0 によるソース公開義務を回避したい企業向けに、あえて商用ライセンス枠を設け、それを現在は無料で配布しています。これは「知財を保護しつつ、普及を優先する」ための戦略的判断です。
  - **継続利用料金**: 将来的に 月額 1,000 円 / 年額 10,000 円 程度を想定（現在は無料）。
- **Personal/OSS/Non-Profit**: 個人利用、教育目的、およびOSSプロジェクトでの利用は、**永続的に無料（AGPL-3.0）** です。
- **開発者への支援（OFUSE）**:
  Stripe や GitHub Sponsors が利用できないため、金銭的なご支援をいただける場合は **[OFUSE](https://ofuse.me/21cfc1d2)** を通じて受け付けています。
