...
| 6 | **Enterprise Add-on** | 有料ユーザー（独自機能） | 専用ダウンロードURL | 💡 構想段階 |
...
### Phase 6: 有料版デリバリー基盤 🔧 雛形完成
- [x] 分離リポジトリ (`ripen-enterprise`) のボイラープレート作成
- [x] Keygen.sh を用いたライセンス認証エンジンの基本実装
- [x] プラグイン方式による Core 版との動的統合メカニズムの確立
- [ ] Lemon Squeezy との Webhook 連携サーバーの構築
...
### 商用ライセンスの考え方
- **維持費が十分に低い場合**（年間1万円以下程度）：商用の無料ライセンス期間を3年でも5年でも提供する。自分に損害がないので。
- **維持費が高くなった場合**：以下のいずれかを検討する。
  - GitHub Sponsors 等による寄付の受付
  - 1社あたり月額1万円程度の商用ライセンス販売
  - エンタープライズ向け個別見積もり
- **いずれの場合も、個人利用・教育・OSSプロジェクトは常に無料（AGPL-3.0）。**
