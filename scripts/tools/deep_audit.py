...
def audit_db(name, path):
    ...
    try:
        ...
        # 2. Detailed Stats
        for table in tables:
            if table.endswith('_fts') or table.endswith('_data') or table.endswith('_idx') or table.endswith('_docsize') or table.endswith('_config'):
        ...
        if name == "Knowledge DB":
            ...
            cursor = conn.execute("SELECT COUNT(*) FROM embeddings WHERE content_id NOT IN (SELECT name FROM entities) AND content_id NOT IN (SELECT filename FROM bank_files)")
            ...
            cursor = conn.execute("SELECT COUNT(*) FROM relations WHERE subject NOT IN (SELECT name FROM entities) OR object NOT IN (SELECT name FROM entities)")
            broken_rel = cursor.fetchone()[0]
            if broken_rel > 0:
                print(f"  [QUALITY WARNING] {broken_rel} relations pointing to non-existent entities!")
            else:
        ...
        if name == "Thoughts DB":
            ...
            if real_count != fts_count:
                print(f"  [QUALITY WARNING] FTS5 index desync! History: {real_count}, FTS: {fts_count}")
            else:
