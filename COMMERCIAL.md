...
### なぜ今、商用ライセンスを「無料」で配っているのか？
本プロジェクトのソースコードは **AGPL-3.0** で公開されています。これは「勝手な商用利用やコードの盗用」を防ぐためにあえて選択した強力なライセンスです。
一方で、企業ユーザーが法務上のリスク（ソース公開義務）を回避しつつ、安心して Ripen を導入できるように、あえて「商用ライセンス（ソース非公開権）」を切り出し、それを現在は無料で提供することで、普及と知財保護を両立させています。

### STEP 1: ライセンスの申請（手動発行）
商用利用（ソース非公開での利用）を開始する際、および無料の商用ライセンスファイル（Keygen.sh 発行）の受け取りを希望される場合は、以下のいずれかの方法でご連絡ください。

- **Email**: [cwblog69@gmail.com](mailto:cwblog69@gmail.com) 宛に、社名またはお名前を送信。
- **GitHub**: [ayato-labs/ripen](https://github.com/ayato-labs/ripen) の Discussion または Issue にて「Commercial License Request」と投稿。
*※自動発行システムは現在準備中（再構築中）です。ご連絡をいただき次第、開発者が手動でライセンスファイルを発行して返信いたします。*
*※Keygen.sh の無料枠の制限（最大100件）のため、発行数が上限に達した場合は、新規のライセンス発行を一時的に停止、または古い無効なライセンスを整理するまでお待ちいただく場合があります。あらかじめご了承ください。*
