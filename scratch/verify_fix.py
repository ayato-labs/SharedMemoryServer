...
class TestErrorHandling(unittest.IsolatedAsyncioTestCase):
    async def test_db_init_failure_message(self):
        ...
        with patch(
            "shared_memory.logic.init_db", side_effect=Exception("Simulated DB Init Failure")
        ):
    ...
    async def test_api_failure_message(self):
        ...
        with patch("shared_memory.logic.init_db", return_value=None):
            with patch(
                "shared_memory.logic.compute_embeddings_bulk",
                side_effect=Exception("API Key Invalid or Timeout"),
            ):
                res = await logic.save_memory_core(
                    entities=[{"name": "Test", "description": "desc"}]
                )
    ...
    async def test_read_memory_db_locked_message(self):
        ...
        with patch("shared_memory.logic.init_db", return_value=None):
            with patch(
                "shared_memory.search.perform_search",
                ...
                self.assertEqual(
                    res, "Database Error: Database is currently locked by another process."
                )
